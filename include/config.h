...
#define PIN_OLED_SCL 33         // Right side — I²C SCL
#define PIN_LED_LOAD1_GREEN 17  // green  = Load1 closed
#define PIN_LED_LOAD1_YELLOW 16 // yellow = Load1 open
#define PIN_LED_LOAD2_GREEN 18  // green  = Load2 closed
#define PIN_LED_LOAD2_YELLOW 19 // yellow = Load2 open
...
//
//  UV_INSTANT  : <150V — supply near collapse, motor winding destruction
//  imminent UV_FAULT    : <207V — IS 12360 -10% hard limit, motor overheating
//  zone UV_WARN     : <216V — CEA -6% supply deviation, first alert RANGE_A_LO
//  : 218.5V — ±5% preferred band lower NOMINAL     : 230V RANGE_A_HI  : 241.5V
//  — ±5% preferred band upper OV_WARN     : >243V — CEA +6% supply deviation,
//  first alert OV_FAULT    : >253V — IS 12360 +10% hard limit, equipment damage
//  zone OV_INSTANT  : >270V — approaching MOV MCOV (275V), zero-debounce trip
...
#define CURR_SC_RUNNING_INSTANT_A 27.0f // Low-set near ADC saturation — SC trip
#define CURR_SC_STARTUP_INSTANT_A 110.0f // High-set during motor startup
#define CURR_SC_HARD_A 120.0f            // Always active catastrophic short
#define CURR_IDLE_LIMIT_A 0.5f // Current threshold to transition out of IDLE
...
//    This UV is caused by the load itself during startup. It is NOT a grid
//    fault. FIX: UV_WARN and UV_FAULT are also suppressed during inrush blank
//    window. UV_INSTANT (<150V) is NEVER suppressed — supply collapse is always
//    real.
//
...
//    SC uses slope logic: if current is RISING after 300ms, it is SC not
//    inrush.
//
...
#define DELAY_MILD_MS 60000      // Severity < 1.5 (Minor grid transient)
#define DELAY_MODERATE_MS 120000 // Severity 1.5 - 2.5 (Moderate severity)
#define DELAY_SEVERE_MS 180000   // Severity > 2.5 (Extreme anomaly)
#define DELAY_PENALTY_MS 60000   // Added if grid is still unstable after wait
...
//  P3b:Supply collapse UV <150V→ instant FAULT, LOCKOUT (motor winding
//  destruction) P4: Thermal limit           → FAULT → LOCKOUT (fire risk) P5:
//  Sustained overvoltage   → FAULT via debounce P6: IDMT overcurrent        →
//  FAULT via accumulator P7: Undervoltage            → FAULT via debounce
//  (motor protection)
//
...
//
//  LOCKOUT-class bits (FSM bypasses auto-reclose, routes directly to LOCKOUT):
//    FAULT_BIT_SENSOR, FAULT_BIT_THERMAL, FAULT_BIT_SC, FAULT_BIT_UV_INSTANT
//  These bits are only cleared by clearAll() (manual LOCKOUT reset path).
//  clearLatched() (auto-reclose path) does NOT clear lockout-class bits.
...
#define FAULT_BIT_UV_INSTANT                                                   \
  0x0080U                         // P3b — supply collapse UV <150V → LOCKOUT
                                  //        Near-total supply collapse; motor
                                  //        winding destruction imminent.
                                  //        Dedicated bit (not shared with
                                  //        FAULT_BIT_UV) so ANSI 27 vs
                                  //        near-collapse is distinguishable
                                  //        in the fault log.  Lockout-class:
                                  //        only clearAll() may clear this bit.
...
#define FAULT_BIT_UV 0x0040U // P7  — undervoltage (sustained, IS 12360 -10%)
...
// recovery delay. Map it to the severe delay so the JSON remains valid.
// The FSM itself uses dynamic computed times.
#define RECOVERY_DELAY_MS DELAY_SEVERE_MS
...
// ─── Temperature Sensor
// ─────────────────────────────────────────────────────── Non-blocking
// conversion: request → wait 800ms → read At 2000ms interval: effectively 0.5Hz
// sampling (sufficient for thermal mass)
...
#define WS_PUSH_INTERVAL_MS 1000 // push telemetry to browser every 1000ms (1Hz)
// ─────────────────────────────────────────────────────────────────────────────
