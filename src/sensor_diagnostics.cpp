...
//  task_comms (via buildJSON) AND from the lwIP HTTP handler
//  (/api/diagnostics). This double-advanced pq_idx and thermal
//  counters, producing double-counted fault events and torn reads.
//
//  After: Three responsibilities separated:
//
//  update(v, i, t)  — Exclusive mutator. Called ONLY from
//    task_comms, once per comms loop. Advances all sliding windows,
//    builds a complete snapshot into the INACTIVE buffer, then
//    atomically swaps the active index with memory_order_release.
//    The MEMW barrier ensures all payload bytes are globally
//    committed before the index flip is visible to any reader.
//    configASSERT in debug builds enforces the ≥800ms call guard.
//
//  lastSnapshot()   — Pure observer. Loads the active index with
//    memory_order_acquire (MEMW barrier prevents speculative
//    prefetch of the wrong buffer) and returns a copy. Lock-free,
//    wait-free. Safe from any context including lwIP callbacks.
//
//  compute(v, i, t) — Backward-compatible shim for existing callers
//    (telemetry_builder.cpp). Delegates to update() then returns
//    lastSnapshot(). Must only be called from task_comms.
//
//  ALL INTERNAL HELPER FUNCTIONS UNCHANGED from v2.0:
//    computeSNR, estimateLinearityError, scoreVoltage, scoreCurrent,
//    scoreThermal, updateThermalState, computeThermalVariance,
//    pushPowerQualityWindow, computePowerQuality, computeOverallScore
// ============================================================
...
namespace {
    ...
    // ── Thermal state mutation (called only from update()) ────────────────
    ...
    // ── Power quality window mutation (called only from update()) ─────────
    ...
    PowerQuality computePowerQuality(float voltage_v, float current_a) {
        ...
        pq.power_factor_estimated = 0.85f;
    ...
    // Builds a complete DiagnosticsSnapshot from current ADCSampler / DS18B20
    // readings and the already-advanced sliding windows. Writes into target.
    // Called exclusively from update(), which owns the inactive buffer write.
    void buildSnapshotInto(DiagnosticsSnapshot& snap,
                           float voltage_v, float current_a, float temp_c) {
        ...
        // ── Voltage Health ────────────────────────────────────────────────
        {
            ...
            snap.voltage.peak_to_peak_v     = snap.voltage.max_seen_v -
                                              snap.voltage.min_seen_v;
        ...
        // ── Thermal Health ────────────────────────────────────────────────
        // Note: updateThermalState() was already called at the top of update()
        // to advance the thermal sliding window. We just read the post-advance
        // values here.
        {
            bool valid = (temp_c > -100.0f) && DS18B20::isReady();
        ...
        // ── ADC Hardware Health ───────────────────────────────────────────
        {
            ...
            float rate_dev_pct  = (expected_rate > 0.0f) ?
                fabsf(actual_rate - expected_rate) / expected_rate * 100.0f : 0.0f;
            const char* cal_labels[] = { "NONE", "LINE_FITTING", "CURVE_FITTING" };
        ...
        // ── System Diagnostics ────────────────────────────────────────────
        {
            ...
            if      (snap.system.overall_health_score >= 85)
                snap.system.health_status = "HEALTHY";
            else if (snap.system.overall_health_score >= 60)
                snap.system.health_status = "DEGRADED";
            else
                snap.system.health_status = "CRITICAL";
        }
    }
...
namespace SensorDiagnostics {
    ...
    // ── MUTATOR — call ONLY from task_comms ──────────────────────────────
    // Advances all sliding windows, builds the snapshot into the inactive
    ...
    void update(float voltage_v, float current_a, float temp_c) {
        ...
        // ── Step 1: Advance all sliding windows (mutation) ────────────────
        ...
        // ── Step 2: Identify inactive buffer ─────────────────────────────
        // Relaxed load is safe here — we are the only writer. The reader
        // (lastSnapshot) only reads, never modifies s_active_idx.
        uint8_t active_idx   = s_active_idx.load(std::memory_order_relaxed);
        uint8_t inactive_idx = active_idx ^ 1u;

        // ── Step 3: Build snapshot into inactive buffer ───────────────────
        // The reader cannot see this buffer during the write because
        // s_active_idx still points to the old active buffer.
        buildSnapshotInto(s_buffers[inactive_idx], voltage_v, current_a, temp_c);

        // ── Step 4: Atomic swap — make the new buffer the active one ──────
        // memory_order_release generates a Xtensa MEMW barrier, ensuring
        // all payload writes to s_buffers[inactive_idx] are globally
        // visible across both cores before the index flip is committed.
        s_active_idx.store(inactive_idx, std::memory_order_release);
    }

    // ── OBSERVER — lock-free, safe from any context ───────────────────────
    // Loads the active buffer index with memory_order_acquire (MEMW barrier
    // prevents the CPU from speculatively loading the wrong buffer's payload
    // before the index read is resolved), then returns a copy.
    DiagnosticsSnapshot lastSnapshot() {
        uint8_t idx = s_active_idx.load(std::memory_order_acquire);
        return s_buffers[idx]; // copy — caller owns the result
    }

    // ── BACKWARD-COMPATIBLE SHIM — call only from task_comms ─────────────
    // Delegates to update() + lastSnapshot(). Existing callers
    // (telemetry_builder.cpp) continue to compile unchanged.
    DiagnosticsSnapshot compute(float voltage_v, float current_a, float temp_c) {
        update(voltage_v, current_a, temp_c);
        return lastSnapshot();
    }
