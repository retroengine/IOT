...
import {
  ...
  onMqttStateChange,
  MQTT_STATE,
} from '../telemetry/telemetryPoller.js';
...
@media (max-width: 640px) {
  .p4-cred-form { grid-template-columns: 1fr; }
}
.p4-cred-field {
  display: flex;
...
.p4-cred-input:focus {
  border-color: color-mix(in srgb, var(--wave-voltage) 60%, transparent);
}
.p4-cred-input::placeholder { color: var(--text-faint); }
...
.p4-btn:disabled { opacity: 0.4; cursor: not-allowed; }
.p4-btn--connect {
  background: var(--health-excellent);
  color: #060f06;
}
.p4-btn--connect:not(:disabled):hover {
...
.p4-cred-status.ok      { color: var(--health-excellent); }
.p4-cred-status.error   { color: var(--state-fault); }
.p4-cred-status.pending { color: var(--state-warning); }
...
export class Page4Cloud {
  ...
  constructor() {
    ...
    // Credential form DOM refs
    this._credStatusEl    = null;
    this._connectBtnEl    = null;
    this._disconnectBtnEl = null;
    this._credInputs      = {};   // { brokerUrl, username, password, topicFilter }

    // MQTT state listener unsubscribe fn
  ...
  mount(containerEl) {
    ...
    this._buildCredentialCard(containerEl);   // Row 0 — NEW
    this._buildStatusCard(containerEl);
    this._buildSignalPathCard(containerEl);
    this._buildInspectorCard(containerEl);

    // Subscribe to MQTT state transitions for live feedback
    this._unsubMqtt = onMqttStateChange((state, detail) => {
      this._onMqttStateChange(state, detail);
    });

    // Sync UI with current MQTT state on mount (e.g. already connected)
    this._onMqttStateChange(getMqttState(), {});
  }
  ...
  destroy() {
    if (this._unsubMqtt) { this._unsubMqtt(); this._unsubMqtt = null; }
  ...
  // ══════════════════════════════════════════════════════════════════════
  // Row 0 — MQTT Credentials (new — Phase 7)
  // ══════════════════════════════════════════════════════════════════════

  // localStorage keys (password is NEVER stored)
  static LS_BROKER   = 'sgs_mqtt_broker';
  static LS_USER     = 'sgs_mqtt_user';
  static LS_TOPIC    = 'sgs_mqtt_topic';

  _buildCredentialCard(parent) {
    const card = _el('div', 'p4-card');
    card.appendChild(_el('div', 'p4-card-hdr', 'BROKER CONNECTION'));

    const form = _el('div', 'p4-cred-form');

    const fields = [
      {
        key:         'brokerUrl',
        label:       'Broker URL (WSS)',
        type:        'text',
        placeholder: 'wss://your-cluster.hivemq.cloud:8884/mqtt',
        store:       Page4Cloud.LS_BROKER,
        full:        true,
      },
      {
        key:         'username',
        ...
        store:       Page4Cloud.LS_USER,
        full:        false,
      },
      {
        key:         'password',
        label:       'Password (session only — never stored)',
        type:        'password',
        placeholder: '••••••••',
        store:       null,          // intentionally not persisted
        full:        false,
      },
      {
        key:         'topicFilter',
        label:       'Topic Filter',
        type:        'text',
        placeholder: 'sgs/device/+/telemetry',
        store:       Page4Cloud.LS_TOPIC,
        full:        false,
      },
    ];
    ...
    for (const f of fields) {
      const wrap = _el('div', f.full ? 'p4-cred-field full-width' : 'p4-cred-field');
      wrap.appendChild(_el('label', 'p4-cred-label', f.label));

      const input = _el('input', 'p4-cred-input');
      input.type        = f.type;
      input.placeholder = f.placeholder;
      input.autocomplete = 'off';
      input.spellcheck  = false;

      // Pre-fill from localStorage (only non-sensitive fields)
      if (f.store) {
        const saved = localStorage.getItem(f.store);
        if (saved) input.value = saved;
      }

      wrap.appendChild(input);
      form.appendChild(wrap);
      this._credInputs[f.key] = input;
    }

    card.appendChild(form);

    // Actions row
    const actions = _el('div', 'p4-cred-actions');

    this._connectBtnEl = _el('button', 'p4-btn p4-btn--connect', '⚡ Connect');
    this._connectBtnEl.addEventListener('click', () => this._onConnectClick());

    this._disconnectBtnEl = _el('button', 'p4-btn p4-btn--disconnect', 'Disconnect');
    this._disconnectBtnEl.addEventListener('click', () => this._onDisconnectClick());

    this._credStatusEl = _el('span', 'p4-cred-status', 'Not connected');

    actions.appendChild(this._connectBtnEl);
    actions.appendChild(this._disconnectBtnEl);
    actions.appendChild(this._credStatusEl);
  ...
  _onConnectClick() {
    const brokerUrl   = this._credInputs.brokerUrl?.value.trim()   || '';
    const username    = this._credInputs.username?.value.trim()     || '';
    const password    = this._credInputs.password?.value           || '';
    const topicFilter = this._credInputs.topicFilter?.value.trim() || 'sgs/device/+/telemetry';

    if (!brokerUrl) {
      this._setCredStatus('Broker URL is required', 'error');
      return;
    }
    ...
    localStorage.setItem(Page4Cloud.LS_BROKER, brokerUrl);
    localStorage.setItem(Page4Cloud.LS_USER,   username);
    localStorage.setItem(Page4Cloud.LS_TOPIC,  topicFilter);

    this._setCredStatus('Connecting…', 'pending');
    connectMqtt({ brokerUrl, username, password, topicFilter });
  }

  _onDisconnectClick() {
    disconnectMqtt();
    this._setCredStatus('Disconnected', '');
  }

  _setCredStatus(msg, cls) {
    if (!this._credStatusEl) return;
    this._credStatusEl.textContent = msg;
    this._credStatusEl.className   = `p4-cred-status${cls ? ' ' + cls : ''}`;
  }

  _onMqttStateChange(state, detail) {
    // Update credential card feedback and button states
    const isConnected   = state === MQTT_STATE.CONNECTED;
    const isConnecting  = state === MQTT_STATE.CONNECTING || state === MQTT_STATE.RECONNECTING;

    if (this._connectBtnEl)    this._connectBtnEl.disabled    = isConnected || isConnecting;
    if (this._disconnectBtnEl) this._disconnectBtnEl.disabled  = !isConnected && !isConnecting;

    const STATUS_MAP = {
      [MQTT_STATE.IDLE]:         ['Not connected',                   ''],
      [MQTT_STATE.CONNECTING]:   ['Connecting…',                     'pending'],
      [MQTT_STATE.CONNECTED]:    ['Connected — receiving telemetry', 'ok'],
      [MQTT_STATE.RECONNECTING]: [`Reconnecting (${detail.delayMs ?? '?'}ms)…`, 'pending'],
      [MQTT_STATE.DISCONNECTED]: ['Disconnected',                    ''],
      [MQTT_STATE.ERROR]:        [`Error: ${detail.reason ?? 'unknown'}`, 'error'],
    };

    const [msg, cls] = STATUS_MAP[state] ?? ['Unknown state', ''];
    this._setCredStatus(msg, cls);

    // Mirror to connection dot when MQTT is the active transport
    if (isConnected) this._updateConnDot(true);
    if (state === MQTT_STATE.DISCONNECTED || state === MQTT_STATE.ERROR) {
      this._updateConnDot(false);
    }
  }
